...
function addQuote() {
  ...
  // Trigger immediate sync with server by posting to the mock API
  postQuoteToServer(newQuote);

  // Display random quote after adding
  showRandomQuote();
}

// Posting new quote to the mock API (mock API endpoint: API_URL)
async function postQuoteToServer(newQuote) {
  try {
    ...
    console.log('New quote posted to server:', responseData);

    // Update sync status after posting successfully
    updateSyncStatus('Quote posted to server successfully.');
  } catch (error) {
    console.error('Error posting quote to server:', error);
    updateSyncStatus('Failed to post quote to the server.', true);
  }
}
